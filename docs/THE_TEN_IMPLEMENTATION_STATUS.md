...
- Formal assessment taking uses the learner shell and retains existing action bindings and `q_<question_version_id>` fields. Pending submission disables edits; keyboard focus is visible on answer labels. Invalidated attempts no longer display a success state. Closed/unopened checkpoints show an explanation.
...
- Existing Supabase auth client, claims checks, session refresh, RLS, migrations, and server actions are unchanged.
- Journey reads use active learner cohort memberships and the signed-in user's client. Attempts/attendance are explicitly filtered to that user and the listed cohort content.
...
- `node --experimental-strip-types scripts/test-the-ten.mjs`: passed. Covers checkpoint windows, invalidation/release gates, next-action selection, feedback identity checks, retry, duplicate submission, locked answers, and released-score bounds.
- Isolated Chromium component harness with synthetic data: desktop 1440px, tablet 768px, mobile 390px and 320px; no page overflow or missing-image requests. Checked skip link/account menu, keyboard answer selection/focus, all feedback outcomes, locked steps, selection review, failed submission/retry, reduced motion, and unchanged assessment FormData during pending submission.
...
4. Run authenticated integration QA for active learner memberships, released results, case delivery, and certificate states.
...
- Existing route/navigation integration: `src/app/assessments/[assessmentId]/take/page.tsx`, `src/app/dashboard/page.tsx`, `src/components/app-shell.tsx`.
...
- Presentation/data modules under `src/lib/the-ten/`: `journey.ts`, `learner-data.ts`, `case-flow.ts`, `feedback.ts`, `motion.ts`, and `assets.ts`.
...
- No changes to SQL/schema, existing server actions, Supabase clients, auth/session helpers, or grading/AI services. New server-side learner queries read existing records through the authenticated client; assessment fields and action bindings are preserved.
