  ...
  const content = (
    <article className={`group rounded-[26px] border p-5 shadow-[0_12px_38px_rgba(23,54,58,0.06)] transition duration-200 motion-reduce:transform-none motion-reduce:transition-none ${stateStyles[state]} ${href && state !== 'locked' ? 'hover:-translate-y-1 hover:shadow-[0_18px_46px_rgba(23,54,58,0.10)]' : ''}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="text-xs font-black uppercase tracking-[0.1em] text-[#637779]">{statusLabel ?? state.replace('_', ' ')}</span>
      </div>
      <h3 dir="auto" className="mt-4 text-lg font-black text-[#17363A]">{title}</h3>
      {description ? <p dir="auto" className="mt-2 text-sm leading-6 text-[#5B7072]">{description}</p> : null}
      ...
      {href && state !== 'locked' && <p className="mt-4 text-sm font-bold text-[#1F6668]">{actionLabel} <span aria-hidden="true">→</span></p>}
    </article>
  )

  if (href && state !== 'locked') {
    return <Link href={href} className="ten-case-link block rounded-[26px]">{content}</Link>
  }
