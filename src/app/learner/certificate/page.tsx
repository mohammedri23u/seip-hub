...
export default async function CertificatePage({ searchParams }: { searchParams: Promise<{ feedback?: string; issued?: string; error?: string }> }) {
  ...
  return <LearnerShell active="/learner/certificate" title="Completion pathway" intro="The certificate is a final recorded state. It unlocks only when every configured requirement is complete.">
    ...
    <section className="rounded-[2rem] border border-[#315b5d] bg-[#17363a] p-6 text-white shadow-[0_24px_70px_rgba(23,54,58,.16)] sm:p-8"><p className="text-xs font-black tracking-[.18em] text-[#f2d99b]">THE FINAL GATE</p><h2 className="mt-3 font-serif text-3xl sm:text-4xl">{summary.eligible || summary.certificate ? 'The Nexus recognizes your completed path.' : 'Your signal map is still forming.'}</h2><p className="mt-4 max-w-2xl leading-7 text-[#d8e7e2]">This pathway reads persisted orientation, assessment, mission, attendance, and feedback records. Page visits alone cannot unlock it.</p></section>
