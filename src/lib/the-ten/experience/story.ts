...
export function createMissionPrelude(input: MissionEpisodeInput): StoryDefinition {
  ...
  const scenes: StoryScene[] = [
    ...
    { id: 'reasoning-lens', eyebrow: `${input.mentor.toUpperCase()} · REASONING LENS`, title: input.lens, narration: 'Carry this way of thinking into the mission. It is a lens, not an answer.', visualKey: 'nexus', character: guardian ?? undefined, characterReaction: 'guide', layout: guardian ? 'portrait' : 'center', motion: 'focus', atmosphere: 'nexus', ctaLabel: 'Prepare to reason' },
...
export function createMissionEpilogue(input: MissionEpisodeInput): StoryDefinition {
  return { id: `mission:${input.runId}:epilogue`, title: `${input.title} epilogue`, version: 1, replayable: true, scenes: [
    { id: 'consequence', eyebrow: 'THE PATH HOLDS', title: 'A connection has been restored.', narration: 'Not because every decision was effortless, but because evidence, judgment and reassessment remained connected.', visualKey: 'nexus', nexusLevel: input.nexusLevel, character: input.guide ?? undefined, characterReaction: 'guide', layout: input.guide ? 'portrait' : 'center', motion: 'focus', atmosphere: 'nexus', ctaLabel: 'See what remains', allowPrevious: false },
