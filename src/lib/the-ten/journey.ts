...
export function checkpointState(assessment: LearnerAssessment, attempt?: LearnerAttempt, now = Date.now()): CheckpointState {
...
export function nextJourneyAction(data: JourneyData, now = Date.now()) {
  const checkpoints = data.assessments.map(assessment => ({ assessment, presentation: checkpointState(assessment, data.attempts.find(a => a.assessment_id === assessment.id), now) }))
  ...
  const live = data.sessions.find(s => s.status === 'live')
  ...
  const upcoming = data.sessions.find(s => s.status === 'scheduled')
