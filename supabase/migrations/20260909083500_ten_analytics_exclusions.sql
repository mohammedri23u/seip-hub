create table if not exists public.program_analytics_exclusions (
...
alter table public.program_analytics_exclusions enable row level security;

create policy "program analytics exclusions read"
on public.program_analytics_exclusions
for select
to authenticated
using (private.is_platform_admin() or private.has_program_role(program_id, array['program_director','assessment_lead']));

create policy "program analytics exclusions manage"
on public.program_analytics_exclusions
for all
to authenticated
using (private.is_platform_admin() or private.has_program_role(program_id, array['program_director']))
with check (private.is_platform_admin() or private.has_program_role(program_id, array['program_director']));

insert into public.program_analytics_exclusions(program_id,user_id,reason,created_by)
select p.id,u.id,'QA / synthetic test account',director.user_id
from public.programs p
join auth.users u on u.email in ('qa.facilitator.seip@example.com','qa.learner1.seip@example.com','qa.learner2.seip@example.com')
join lateral (
  select pm.user_id from public.program_memberships pm
  where pm.program_id=p.id and pm.role='program_director' and pm.status='active'
  order by pm.created_at limit 1
) director on true
where p.code='SEIP26'
on conflict (program_id,user_id) do update set reason=excluded.reason;
...
begin
  ...
  with settings as (
    ...
    left join public.learner_assessment_results lar on lar.assessment_id in (s.pre_assessment_id,s.post_assessment_id)
      and not exists (select 1 from excluded e where e.user_id=lar.learner_id)
  ), objective_rows as (
    ...
    left join public.learner_assessment_results lar on lar.id=lor.assessment_result_id
      and lar.assessment_id in (s.pre_assessment_id,s.post_assessment_id)
      and not exists (select 1 from excluded e where e.user_id=lar.learner_id)
    ...
    where lpf.program_id=target_program_id
      and not exists (select 1 from excluded e where e.user_id=lpf.learner_id)
  ), latest_consent as (
    select distinct on (rcr.learner_id) rcr.learner_id,rcr.status
    from public.research_consent_records rcr
    where rcr.program_id=target_program_id
      and not exists (select 1 from excluded e where e.user_id=rcr.learner_id)
    order by rcr.learner_id,rcr.recorded_at desc
  ), consent_summary as (
    select count(*) filter (where lc.status='consented') as consented,
