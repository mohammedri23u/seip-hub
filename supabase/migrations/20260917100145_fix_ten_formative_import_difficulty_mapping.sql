...
begin
  ...
  loop
    item_type := item->>'type';
    normalized_difficulty := case
      when lower(coalesce(item->>'difficulty','')) like '%expert%' then 'expert'
      when lower(coalesce(item->>'difficulty','')) like '%hard%' then 'hard'
      when lower(coalesce(item->>'difficulty','')) like '%easy%' and lower(coalesce(item->>'difficulty','')) not like '%moderate%' then 'easy'
      else 'moderate'
    end;
